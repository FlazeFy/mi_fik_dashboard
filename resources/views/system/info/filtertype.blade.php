<div class="position-absolute" style="top:65px; left:110px;">
    <div class="mb-2" style="font-size:15px;">
        ...
        <form action="/system/info/filter_type" method="POST">
            @csrf
            <select class="form-select" style="font-size:14px; padding:4px auto;" id="info_type" name="info_type" onchange="this.form.submit()"  aria-label="Floating label select example" required>
                @php($i=0)
                @foreach($dct as $dtag)
                    @if($i == 0)
