...
@if($str[0] == "created_at" && $str[1] == "ASC")
    <form class="d-inline" action="/user/all/ordered/DESC/joined" title="{{ __('messages.sort_asc_join') }}" method="POST">
        @csrf
...
@elseif($str[0] == "created_at" && $str[1] == "DESC")
    <form class="d-inline" action="/user/all/ordered/ASC/joined" title="{{ __('messages.sort_desc_join') }}" method="POST">
        @csrf
        <button class="btn btn-primary px-2 py-0 ms-2" type="submit"><i class="fa-solid fa-sort fa-sm"></i></button>
    </form>
@else
