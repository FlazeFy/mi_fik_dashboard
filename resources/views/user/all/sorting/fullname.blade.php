...
@if($str[0] == "first_name" && $str[1] == "ASC")
    <form class="d-inline" action="/user/all/ordered/DESC/first_name" title="Sort fullname by ascending" method="POST">
        @csrf
...
@elseif($str[0] == "first_name" && $str[1] == "DESC")
    <form class="d-inline" action="/user/all/ordered/ASC/first_name" title="Sort fullname by descending" method="POST">
        @csrf
        <button class="btn btn-primary px-2 py-0 ms-2" type="submit"><i class="fa-solid fa-sort fa-sm"></i></button>
    </form>
@else
