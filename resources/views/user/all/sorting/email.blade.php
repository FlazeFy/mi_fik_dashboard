...
@if($str[0] == "email" && $str[1] == "ASC")
    <form class="d-inline" action="/user/all/ordered/DESC/email" title="Sort email by ascending" method="POST">
        @csrf
...
@elseif($str[0] == "email" && $str[1] == "DESC")
    <form class="d-inline" action="/user/all/ordered/ASC/email" title="Sort email by descending" method="POST">
        @csrf
        <button class="btn btn-primary px-2 py-0 ms-2" type="submit"><i class="fa-solid fa-sort fa-sm"></i></button>
    </form>
@else
