...
@if($str[0] == "group_desc" && $str[1] == "ASC")
    <form class="d-inline" action="/user/group/ordered/DESC/group_desc" title="Sort group description by ascending" method="POST">
        @csrf
...
@elseif($str[0] == "group_desc" && $str[1] == "DESC")
    <form class="d-inline" action="/user/group/ordered/ASC/group_desc" title="Sort group description by descending" method="POST">
        @csrf
        <button class="btn btn-primary px-2 py-0 ms-2" type="submit"><i class="fa-solid fa-sort fa-sm"></i></button>
    </form>
@else
