...
<div class="content-image-holder">
    ...
    <div class='image-upload' id='formFileImg' onchange='setValueContentImage()'>
        <label for='file-input'>
            <img class='btn change-image shadow position-relative p-1' title='Change Image' src="{{asset('assets/change_image.png')}}"/>
        </label>
        <input id='file-input' type='file' name='content_image' accept="image/*" value="" onchange='setValueContentImage()'/>
    </div>
...
<script>
    ...
    function clearImage() {
        ...
        document.getElementById('file-input').value = "{{asset('assets/default_content.jpg')}}";
    ...
    function setValueContentImage(){
        ...
        function () {
            uploadTask.snapshot.ref.getDownloadURL().then(function (downloadUrl) {
                var cheader_preview_elmt = "<img class='img img-fluid mx-auto rounded mt-2' src='" + downloadUrl + "' alt='" + downloadUrl + "'>";
               
                document.getElementById('frame').src = downloadUrl;
                document.getElementById('file-input').value = downloadUrl;
            });
        });
    }
</script>
